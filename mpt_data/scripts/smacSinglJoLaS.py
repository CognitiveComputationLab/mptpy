...
def error (x1, x2, x3):
	...
	params[2] = x3
	tree = [0] *4
	tree[0] = (1-params[0]) * (1-params[1])
	tree[1] = params[0] * params[1] * params[2] + (1-params[0]) * params[1] * params[2]
	tree[2] = params[0] * (1-params[1]) + (1-params[0]) * params[1] * (1-params[2])
	tree[3] = params[0] * params[1] * (1-params[2])
	tree = npy.array(tree)
...
path = "/home/noef/smac-v2.10.03-master-778/hiwi/4patern/4test.csv"#allADG.csv"
dataFile = path
...
for x in range (0,len(fullData)):
	...
	indice = x
	vals, params = opt.minimize(error, 10, parameters)
	paramsArr.append(params)
	print(x)
	x1 = params['x1']
	x2 = params['x2']
	x3 = params['x3']
	restemp = prints(x1, x2, x3)
	resultsArr.append(restemp)
printer = npy.array(resultsArr)
print(paramsArr)
end = time.time()
print(end - start)
npy.savetxt("results/testJLS.csv", printer, delimiter=',')